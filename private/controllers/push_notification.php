...
if(empty($admin)){
    ...
    if (Helper::is_post()) {
        ...
        $push_notification->id = $_GET['id'];
        $push_notification->admin_id = $_GET['admin_id'];
        if(!empty($push_notification->admin_id) && !empty($push_notification->id)){
            if($admin->id == $push_notification->admin_id){
                if($push_notification->where(["id" => $push_notification->id])->andWhere(["admin_id" => $push_notification->admin_id])->delete()){
                    ...
                    $errors->add_error("Error Occurred While Deleting");
                }
            }else{
                $errors->add_error("You re only allowed to delete your own data.");
            }
        }else{
            $errors->add_error("Invalid Notification.");
        }
