...
if(empty($admin)){
    ...
    $restaurant_menu_id = isset($_GET['id']) ? $_GET['id'] : '';
    $all_restaurant_menu = new Res_menu();

    // Items for Edit
    $restaurant_menu = (isset($restaurant_menu_id) && !empty($restaurant_menu_id)) ? $all_restaurant_menu->where(["id" => $restaurant_menu_id])->one() : '';
    $image = (!empty($restaurant_menu) && !empty($restaurant_menu->itm_img_id)) ? $restaurant_menu->image : "default_pic.png";
    $image_path = "/public/tripeasy_images/" . $image;
    $menu_title = (!empty($restaurant_menu) && !empty($restaurant_menu->title)) ? $restaurant_menu->title : "";
    $restaurant_type = (!empty($restaurant_menu) && !empty($restaurant_menu->type)) ? $restaurant_menu->type : "";
    $restaurant_price = (!empty($restaurant_menu) && !empty($restaurant_menu->price)) ? $restaurant_menu->price : "";
    $restaurant_quantity = (!empty($restaurant_menu) && !empty($restaurant_menu->quantity)) ? $restaurant_menu->quantity : "";

    $all_restaurant_details = new Restaurant_details();
    $restaurant_details = ( !empty($restaurant_menu)) ? $all_restaurant_details->where(["id" => $restaurant_menu->res_id])->one() : '';
    $restaurant_title = $restaurant_details->title;

    //$all_restaurants = $all_restaurant_details->all();

    $action = "../private/controllers/restaurantDetailController.php";
    $action = !empty($restaurant_menu_id) ? $action . "?id=" . $restaurant_menu_id : $action;
